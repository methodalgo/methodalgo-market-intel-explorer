...
Use `methodalgo-cli` v1.0.36 or newer for the `macro` and `totals` commands. Macro/FRED-derived data is handled by Methodalgo on the server side, so users do not need to configure a local FRED API key.
